...
void handle_enqueue()
{
    ...
    rear++;
    short elem_pos = rear % queue_size;
    queue_array[elem_pos] = tmp_elem;
...
void handle_dequeue()
{
    ...
    short elem_pos = front % queue_size;
    int tmp = queue_array[elem_pos];
    if(front == rear)
    {
        front = -1;
        rear = -1;
...
void display_elements()
{
    ...
    for(short iter = front;iter <= rear;iter++)
    {
        short elem_pos = iter % queue_size;
        cout << queue_array[elem_pos] << "\t";
    }
}
